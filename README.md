...
- **鍵盤**: 画面の幅に合わせて白鍵 8・15・22・29 本（スマホ横向きで 2 オクターブ＋1、パソコンで 3 オクターブ＋1）。◀ ▶・← → で白鍵 1 つずつ動かす（どの白鍵からでも始められる。例 F3〜F5）。長押し・Shift＋クリック・Shift＋← → で 1 オクターブ。A0〜C8 の中で止まり、黒鍵からは始めない（`piano-core.js` の `shiftStart`・`clampStart`）。MIDI キーボードの音は鍵盤の外でも鳴る（画面は動かさない）
- **タップ**: Pointer Events で指ごとに押している鍵を持つので、複数の指で和音。指をすべらせると隣の鍵へ
- **パソコンのキー**: `KeyboardEvent.code`（キーの物理的な位置）で割り当てる。Z 段＝白鍵・A 段＝黒鍵、Q 段・数字の段＝1 オクターブ上。Z は左はしの白鍵を含むオクターブのド（`keyboardBase`）。左はしの白鍵を Z にするとファ始まりで V が黒鍵になるなど白黒がずれるので、Z はいつもドにする。同じオクターブの中で動かしても割り当ては変わらない。JIS と US で印字が違うのは `BracketLeft`（@／[）・`Equal`（^／=）・`BracketRight`（[／]）の 3 つだけで、画面の文字は `navigator.keyboard.getLayoutMap()` か、押された `key` から合わせる
...
- **そのほか**: 音名（ドレミ／CDE／なし）、サステイン（ボタンか Space を押している間）、音量、4 拍のクリック（BPM 30〜240）。設定（音色・左はしの白鍵 `start` を含む）は `web-metronome_piano` に保存。以前の `start`（ドだけ 24〜96）はそのまま同じドとして読む
- **広告**: 弾く画面（`piano/index.html`）は全画面の楽器なので AdSense の meta だけ。使い方（`piano/guide.html`）は通常の自動広告
- **テスト**: `npm test`（`tests/piano.test.js`。周波数・キーの割り当て・鍵盤の範囲・白鍵 1 つずつの移動と端・音域の表示・音色の設計・MIDI メッセージの読み方・設定の正規化・ページの決まり）
